...
void PDMClass::IrqHandler(bool halftranfer) {
    ...
    if (!_doubleBuffer.available()) {
        ...
        if (filter.Decimation == 128) {
            Open_PDM_Filter_128(rawBuffer[rawBufferIndex], finalBuffer, 20, &filter);
        } else {
            Open_PDM_Filter_64(rawBuffer[rawBufferIndex], finalBuffer, 20, &filter);
        }
